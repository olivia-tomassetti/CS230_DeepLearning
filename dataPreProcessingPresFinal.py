...
yArrays = []  # To store y arrays for all trials
trial_ids_per_trial = []  # matching trial id strings for each entry in yArrays
...
for subj_id in subjects:
    ...
    for trialName, trialDf in subjectTrials.items():
        ...
        # Store per-trial y array and trial id for later lookup
        # (we append the trial id after constructing `trialIDstr` below)
...
# ---------------------------------------------------------------------------------
groups = np.array(trialIDs)
# Your existing combined_stratify (one per window)
y_strat = combined_stratify

# We want a single train/test split with ~20% test
# For example, use 5 folds and pick one fold as test:
...
# Take the first split as our train/test
train_indices, test_indices = next(sgkf.split(X, y_strat, groups=groups))

# Now build sets
X_train = X[train_indices]
Y_train = Y[train_indices]
starts_train = starts[train_indices]
trialIDs_train = groups[train_indices]

X_test = X[test_indices]
Y_test = Y[test_indices]
starts_test = starts[test_indices]
trialIDs_test = groups[test_indices]

print("\nIntersection of train/test trial IDs (should be empty):")
print(np.intersect1d(trialIDs_train, trialIDs_test))
...
# Create concatenated array (useful for legacy loading) and also save per-trial object arrays
concatenatedY = np.concatenate(yArrays) if len(yArrays) > 0 else np.array([], dtype=int)

np.savez_compressed('dataYOrderedByTrial.npz',
                    Ylabels=concatenatedY,
)

# Save per-trial arrays alongside their trial ids (object arrays; use allow_pickle when loading)
